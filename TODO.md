...
- [x] **Phase 1: 2D 고전역학 (Matter.js)** [DONE]
    ...
    - ✅ 웹/모바일 반응형 레이아웃 및 가상 좌표계(800x400) 최적화.
- [x] **Phase 2: 시각적 정교화 및 안정성 (Refinement)** [DONE]
    - ✅ **벡터 라벨 스마트 정렬**: 화살표 라벨 클리핑 방지 및 자동 위치 보정 로직 구현.
    - ✅ **레이어 우선순위**: 라벨/화살표를 최상단 레이어로 배치하여 가독성 극대화.
    - ✅ **지면 시각화**: 가상 좌표계 바닥면(y=400) 가이드라인 추가 및 실제 중력 환경 묘사.
    - ✅ **응답 안정성**: `maxOutputTokens` 상향 및 시각화 중복 생성 방지 지침 강화.
