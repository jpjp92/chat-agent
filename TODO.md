...
- ✅ **ApexCharts 통합**: `scatter`, `radar`, `treemap` 등 신규 차트 타입 3종 추가 및 데이터 정규화 로직 고도화.
- ✅ **화학 구조 시각화 (SMILES)**: 분자 이름 표시, SVG 다운로드 기능, 호버 확대 효과 등 프리미엄 UI 적용.
- ✅ **결과물 내보내기**: 모든 차트 및 화학 구조에 대해 SVG 다운로드 버튼 추가.
- ✅ **LaTeX 렌더링 최적화**: KaTeX CSS 연동, 모바일 가로 스크롤 지원, 인라인/블록 수식 스타일링 차별화.
- ✅ **시스템 프롬프트 강화**: 신규 시각화 유형에 대한 AI 지침 구체화.
