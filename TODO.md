...
### 2. 물리학 시뮬레이션 엔진 도입 (Phy-Viz) [COMPLETED]
- [x] **Phase 1: 2D 고전역학 (Matter.js)** [DONE]
    - ✅ `matter-js` 설치 및 `PhysicsRenderer` 컴포넌트 구현.
    - ✅ Gemini 시스템 프롬프트 연동 및 `json:physics` 블록 처리 로직 탑재.
    - ✅ 웹/모바일 반응형 레이아웃 및 스케일링 최적화.
    - ✅ 투명 경계 및 시각적 노이즈 제거 (Premium Look).
    - 💡 **가능한 시뮬레이션 예시**:
        - 탄성/비탄성 충돌 (공 튀기기, 당구)
        - 투사체 운동 (포사체 발사, 벡터 분해 시각화)
        - 자유 낙하 및 가속도 실험 (G 벡터 표시)
        - 운동량/각운동량 보존 법칙 (회전 관성 시뮬레이션)
