...
- [DEV_260514.md](DEV_260514.md) — **URL 수집 Jina Reader fallback 적용**: Wikidocs 블로그 URL 요약 실패 원인을 테스트 스크립트로 확인. Node 서버 사이드 직접 fetch는 `403 Forbidden` + `Just a moment...` Cloudflare challenge HTML만 수신해 selector/Cheerio 보강으로는 해결 불가. `scripts/test-url-extract.mjs`로 직접 fetch·기존 추출·Wikidocs selector·Cheerio 후보·Jina Reader 후보를 비교했고, Jina Reader는 `200 OK`와 실제 본문 Markdown 약 4.4KB를 확보. `api/fetch-url.ts`에 challenge 감지(`403`, `Just a moment...`, `cf-chl`, `cloudflare`, `challenge-platform`)와 Jina Reader fallback(`https://r.jina.ai/http://{targetUrl}`)을 추가. 직접 수집이 짧을 때는 Jina를 보조 시도하되 실패하면 기존 짧은 본문을 유지. 사용자-facing 원문 직접 수집 실패 안내는 추가하지 않음. `npx tsc --noEmit` 성공.
