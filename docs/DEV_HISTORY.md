...
- [DEV_260506.md](DEV_260506.md) — **전국 약국 조회 통합 및 극단적 속도 최적화** (공공데이터포털 API 연동, `sido`/`sigungu` Zod 스키마 분리, JSON Minify, Tool 내부 자체 Markdown 반환, **Direct Injection으로 LLM 대기시간 소거**, **단일 1000건 호출로 페이지네이션 누락 완벽 해결**, LangChain `maxRetries: 0` 설정, 세부 키워드 필터링 및 0건 검색 시 Google Search Fallback, **행정구역 띄어쓰기 서버단 자동 정제(Sanitize)**, **주말/공휴일 영업 휴일지킴이약국 가중치 정렬**, 약국 영업시간 모바일 UI 반응형 튜닝, **전화번호 클릭 기기별 분기(모바일 다이얼러 / 데스크톱 토스트 복사)**, Toast 멀티라인 컴포넌트 업그레이드) / **외부 API 통합 기획 수립** (약국·병원·문화행사 등 7개 API 구현 Phase 기획) / **`medical_qa` Google Search 개선**
