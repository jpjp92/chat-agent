...
- [DEV_260426.md](DEV_260426.md) — **스트리밍 중 `**` 볼드 마커 dangling 수정** (홀수 `**` 감지 시 닫기 추가, backtick 처리와 동일 패턴, `ChatMessage.tsx` 2곳) / **날씨 이모지 테이블 누락 수정** (예시-가이드 이모지 불일치 해소, 테이블 셀 이모지 MANDATORY 지시 추가, `prompt.ts`) / **MFDS 미등재 약품 검색 폴백 개선** (파스·연고 등 비알약 제형 — Google Search grounding → DuckDuckGo → LLM 내부 지식 3단계 폴백 체인, `searchDrugViaGoogleSearch()` 헬퍼 추가, 시스템 프롬프트 예외 명시, 스트리밍 누출 방어) / **MFDS 폴백 출처 칩 미표시 수정** (`groundingChunks` → `webSearchQueries` 폴백, `on_chain_end` LangGraph ToolMessage 직접 스캔, `sources` 이벤트 텍스트보다 먼저 도착 시 `pendingSources`로 보관 후 메시지 생성 시 반영 — `useChatStream.ts`) / **첨부파일 깨진 이미지 아이콘 대체** (`AttachmentImage` 컴포넌트 추가, `onError` 시 fa-image 폴백, `ChatMessage.tsx`) / **첨부파일 mimeType·fileName 오추론 수정** (`EXT_MIME` 맵 + `inferAttachment` 확장자 우선 추론, `useChatSessions.ts`) / **첨부파일 아이콘 색상 버그 수정** (`text-slate-400` 분리, hwpx 체크 추가) / **스트리밍 `[N]` citation 청크 분할 수정** (lookbehind citationBuffer 패턴으로 청크 경계 strip 누락 해소, `generator.ts`·`chat.ts`)
