...
- [DEV_260506.md](DEV_260506.md) — **외부 API 통합 기획 수립** (약국·병원·문화행사·논문·학교·법령 7개 API, intent 분류 전략, 카드 UI 스펙, JSON 포맷, 구현 Phase 계획 문서화 / `docs/DEV_260506.md`) / **`medical_qa` Google Search 개선** (`generator.ts` — `useGoogleSearch` 강제 ON, `thinkingBudget: 3000` 상한 설정, token budget 8192 / `prompt.ts` — Google Search 기반 가이드라인 7개 명시 / 테스트 결과: sources 12개, kdca.go.kr·amc.seoul.kr 등 신뢰도 높은 출처 칩 정상 표시)
