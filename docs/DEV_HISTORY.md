...
- [DEV_260506.md](DEV_260506.md) — **전국 약국 조회 통합 및 극단적 속도 최적화** (공공데이터포털 API 연동, `sido`/`sigungu` Zod 스키마 LLM 파싱 분리, JSON 공백 최소화(Minify)로 Stream Parse 에러 해결, Hallucination 차단을 위한 Tool 내부 자체 Markdown 블록 반환 패턴 적용, **Direct Injection(Fast-pass)으로 LLM 10초 대기 시간 완전 소거**, **약국 API 1/2페이지 Promise.allSettled 병렬화로 통신 딜레이 반갈죽**, **LangChain `maxRetries: 0` 설정으로 429 에러 시 1분 Hang 증상 차단 및 자체 Round-Robin 즉시 가동**, **세부 키워드 필터링(동/약국명) 및 0건 검색 시 Google Search Fallback 구현**, **약국 영업시간 모바일 UI 줄바꿈 방지 반응형 튜닝**) / **외부 API 통합 기획 수립** (약국·병원·문화행사·논문·학교·법령 7개 API, intent 분류 전략, 카드 UI 스펙, JSON 포맷, 구현 Phase 계획 문서화 / `docs/DEV_260506.md`) / **`medical_qa` Google Search 개선** (`generator.ts` — `useGoogleSearch` 강제 ON, `thinkingBudget: 3000` 상한 설정, token budget 8192 / `prompt.ts` — Google Search 기반 가이드라인 7개 명시 / 테스트 결과: sources 12개, kdca.go.kr·amc.seoul.kr 등 신뢰도 높은 출처 칩 정상 표시)
