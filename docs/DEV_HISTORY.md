...
- [DEV_260512.md](DEV_260512.md) — **모델 레지스트리 중앙화 계획 수립**: 현재 모델 문자열 하드코딩 위치 점검(`App.tsx`, `Header.tsx`, `useChatStream.ts`, `chat.ts`, `state.ts`, `generator.ts`, `router.ts`, `vision.ts`, `drug-info-tool.ts`, `speech.ts`, `summarize-title.ts`) / Phase A 서버 상수화(`api/_lib/models.ts`) + Phase B 프론트 레지스트리(`src/lib/models.ts`) + Phase C build 검증으로 분리 / OpenAI·Gemini 멀티 프로바이더 추상화는 `generator.ts`의 Gemini SDK 결합도 때문에 별도 프로젝트로 보류
