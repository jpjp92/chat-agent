...
- [DEV_260430.md](DEV_260430.md) — **YouTube 요약 처리 성능 개선**: (1) `useChatStream.ts` `fetchUrlContent` + `fetchYoutubeTranscript` 직렬 → `Promise.all` 병렬화 (2~5s 절감) (2) `router.ts` YouTube URL fast-path 추가 — YouTube 감지 시 `gemini-2.5-flash-lite` LLM 분류 호출 생략, 즉시 `general → generator` 라우팅 (1~3s 절감) (3) `fetch-transcript.ts` 서버 timeout 단축 (페이지 12s→6s, XML 15s→8s, 실패 케이스 최대 대기 27s→14s) (4) `geminiService.ts` 클라이언트 timeout 45s→20s / **세션 타이틀 생성 1차 개선**: `summarize-title.ts` 프롬프트 "5단어 이내" → "10단어 이내, 내용을 알 수 있게" 변경, `maxOutputTokens` 120 → 200 / **세션 타이틀 생성 2차 개선**: 어시스턴트 응답 마크다운 제거 + 300자 트림, `제목:` 접미사 제거(헤드라인 단편 스타일 방지), 프롬프트에 미완성 구 금지·완결된 명사형 제목 명시, 단어 수 5~15단어로 재지정
