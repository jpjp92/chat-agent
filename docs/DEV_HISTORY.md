...
- [DEV_260607.md](logs/DEV_260607.md) — **모바일 멀티턴 UI 턴 간격 조정**: 멀티턴 대화에서 AI 응답 후 다음 턴(유저 메시지) 시작 전 시각적 구분이 부족하다는 피드백 대응. `ChatMessage.tsx` 메시지 버블 하단 마진을 역할별로 분리 — AI 응답은 모바일에서 `mb-16`(64px)·데스크톱은 기존 `mb-8`(32px) 유지(`sm:mb-8`), 유저 메시지는 `mb-8` 그대로. 데스크톱 레이아웃 변경 없음, 1줄 수정.
