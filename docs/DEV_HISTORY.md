...
- [DEV_260622.md](logs/DEV_260622.md) — **generator.ts 리팩토링 3-A(LangChain 경로 분리) + drug_info 버그픽스 2건**: **3-A** `langchain-path.ts` `runLangChainPath` — SDK실패 YT가드+fast-pass 6종+direct pill lookup+tool 바인딩·invoke+키회전(기존 L682~881)을 terminal 제어흐름으로 추출, caller는 `return await` 1줄. ChatGoogleGenerativeAI·tool 8종·SystemMessage/HumanMessage·pill-messages import 이전. move-identity diff 일치 + dev E2E 4종(drug_info 카드·pharmacy fast-pass·sports·general SDK회귀) + 동등성 19,423 회귀 0 fail. generator.ts 누적 **1148→685줄(-40%)**, 커밋 `21e2f29`. **버그픽스(pre-existing, 3-A E2E 중 발견)** 커밋 `ceaa37f`: ① OCR 토큰 누출 — `tools` 노드의 `extractImprintViaVision`(이미지 OCR용 중첩 LangChain invoke) 토큰이 부모 SSE로 새어 텍스트 질문 답변 앞에 "JPW" 등 노출 → `chat/route.ts` 스트림 게이트 denylist(`vision`만)→**allowlist(`langgraph_node !== 'generator'`)** 전환. ② 각인 로고 오판 — placeholder "마크"(로고형)를 억지 알파벳 추측해 곡선로고를 M/W/JP로 오판 → "명확한 숫자·문자만, 로고·애매하면 `마크` 유지" + temp 0. 진단로그로 노드명·Vision raw 실증, 파모티딘 재현 누출소멸·각인 `마크` 정상. **+도구 응답 성능 회귀 수정 — 모델 정책 정리 + sports 비스트리밍(§3)**: "파모티딘정" 등 흔한 약명이 프로덕션에서 1분 꽉 채우고 무응답(generateContent 16회)인 것을 출발점으로, 약국·병원·동물병원·영화 등 도구 응답 전반의 저하까지 확장 진단. **회귀 원인**=커밋 `1cd48c2`(5/30 기본모델 2.5→3.5-flash 전환) — Vision OCR fan-out(MFDS 5건×앞뒤=최대 10콜)·LangChain 도구 invoke가 전부 `DEFAULT_CHAT_MODEL`을 써 무거운 3.5로 돌며 지연 급증(Vision은 Vercel 무료티어 60s 초과, 상향 불가). 5/30 이전엔 전부 2.5라 빨랐음. **모델 정책 정리**: 외부 API 도구 인텐트(drug_info/drug_id/pharmacy/hospital/vet/law/movie/sports)는 핵심이 API 데이터지 모델 추론이 아니므로 **2.5-flash**(fast-pass는 thinking off, 합성형은 2.5 기본), 일반 대화(SDK 경로)만 3.5 유지. `extractImprintViaVision` 2.5+`thinkingBudget:0`, `langchain-path` `pathModel` 다운시프트+실제모델 로그. **sports 비스트리밍**: 월드컵 마크다운 표가 셀 단위로 실시간 조립돼 어색 → router intent 캡처 후 sports는 `on_chat_model_stream` 스킵, `on_chain_end` 폴백이 표 전체를 1회 전송. dev 실측: drug_info 18.5s/23.7s(이전 60s+ 무응답)·도구 5~9s·sports 19s(2 invoke 모두 2.5 확인). 느린 케이스는 그 시점 Gemini "high demand" 503 재시도 누적(모델 무관). tsc 0. **+위키독스 URL fetch 재차단 수정(§4)**: wikidocs 요약이 502(보안확인/CAPTCHA). 진단=위키독스 전용 경로의 browserless `/unblock`(datacenter)이 CF 강화로 "Just a moment" 챌린지만 반환(미우회, DEV_260606 당시엔 100%), ScraperAPI 폴백은 키 부재로 스킵 → 단일 의존 붕괴. 해결=env에 있던(미사용) `SCRAPINGBEE_KEY`로 ScrapingBee(`render_js`+`premium_proxy`+`country_code=kr`) CF 우회 성공 확인 → `scrapingBeeFetch()` 추가, `cloudflareUnblock` 체인을 ScrapingBee 1차→browserless 폴백→ScraperAPI로 재정렬, 성공분 url_cache(14일) 저장. 검증: 1차 6.1s(본문 4,178자)→2차 캐시 HIT 0.089s. 주의: ScrapingBee premium+render는 크레딧 큼(캐시로 흡수), 프로덕션 Vercel env에 SCRAPINGBEE_KEY 필요. tsc 0.
- [DEV_260621.md](logs/DEV_260621.md) — **월드컵 sports tool 구현 (football-data.org 연동)**: Gemini grounding이 실시간 스포츠 순위에 부적합(organic 웹만 봄→조별 신선도 불균질·소스 5~25 비결정적, 구글 스포츠 지식 패널 접근 불가 — DEV_260620 §5 진단)한 문제를 전용 API tool로 해결. **신규** `lib/sports/football-data.ts`(WC standings/matches/scorers fetch+멀티그룹 파싱+인메모리 캐시 TTL 300/600s·stale 폴백+마크다운, 미정 대진은 `[NOT_DETERMINED]`) + `server/agent/worldcup-tool.ts`(LangChain tool, `resource` 인자 선택적 fetch, `[WORLDCUP_DATA]`+지시문). **배선**: `state.ts` IntentType `sports`, `graph.ts` ToolNode, `generator.ts` LANGCHAIN_INTENTS+`allTools` 바인딩, `router.ts` sports 의도+**과거 대회(연도/지난·역대)→general 분기**(API는 현재 대회만, 과거는 학습지식), `intentRules.ts` 휴리스틱. **🔴 플랜 갭**: generator `allTools` 분기 sports 바인딩 누락을 실행 중 발견·수정(없으면 tool이 LLM에 안 붙음). 검증: tsc 0, E2E 5종 통과(12조 일관 표·한국 16강 추론·미확정 안내·2022 학습지식·득점왕 한글번역). 제약: 과거시즌 403·rate limit 분당 6회(캐시 필수). 설계 [PLAN_WORLDCUP_SPORTS_TOOL_260621](plans/PLAN_WORLDCUP_SPORTS_TOOL_260621.md)·구현계획 [PLAN_WORLDCUP_IMPL_260621](plans/PLAN_WORLDCUP_IMPL_260621.md). **+버그픽스**: 메가박스 영화 카드 HTML 엔티티 미디코드(`2D&#40;더빙&#41;`) → `app/api/showtimes/route.ts`에 `decodeEntities` 추가, movie·format·cinema 적용(`2D(더빙)`). **+kordoc HWP 파싱 연동(P1~P3, DEV_260620 계획 구현완료)**: HWP 4종(`.hwp/.hwpx/.hwp3/.hwpml`)을 구조 보존 마크다운(진짜 `<table>`)으로 추출. **신규** `app/api/parse-document/route.ts`(4MB 임계값 라우팅: raw≤4MB 직행 multipart / 초과 Storage 경유 `{filePath}` download, `preferredRegion='icn1'`, L4 길이상한 100K자 트렁케이트, L5 blocks[] 미반환) + `ChatInput.tsx` `parseViaKordoc` 헬퍼·HWP 4종 분기(kordoc 우선+`.hwpx` JSZip 폴백)·accept/드롭/아이콘 + `next.config.ts` `serverExternalPackages:['kordoc']`(webpack이 kordoc 내부 pdfjs worker 정적 import 번들하려다 빌드 실패→외부화). **🔴 보안 IDOR 대응(자동 리뷰)**: `{filePath}`가 service-role로 chat-docs 임의 HWP 다운로드·삭제 가능 → ① 경로 형식 엄격 검증(traversal 차단) ② 파괴 벡터인 route-side `remove()` 제거(정리는 버킷 TTL 위임). 근본해결(앱 전역 인증+유저별 prefix)은 백로그. **+search-gate(§7)**: 첨부 문서(`[EXTRACTED_CONTENT:]`/`[PREVIOUSLY_UPLOADED_DOCUMENT_CONTENT:]`) 있으면 grounding 기본 off, 사용자가 외부 검증(검색/최신/출처 등) 명시 요청 시에만 on — 문서 요약에 불필요한 two-track 검색 붙던 문제 해소. 실사용 E2E: 직행 1.7MB·Storage 10.6MB 파싱, 문서 검토 단일패스 14s(grounding suppressed). 검증: tsc 0, build 성공(라우트 13→14). **+DOCX 표 보존(§8)**: docx를 mammoth `extractRawText`→`convertToHtml`로 전환해 표를 `<table>`(th/td) 보존(kordoc HWP와 동일) — 셀이 평문으로 뭉개지던 문제 해소. 검증 `scripts/test-docx-table.mjs`(fixture 표 2개→`<table>`×2·td×26, 레이턴시 +~18ms 무시 가능). **+PPTX 표 보존(§9)**: `<a:tbl>`→`<a:tr>`/`<a:tc>` 구조 파싱으로 마크다운 표 산출(신규 의존성 0). 구조 파악 중 `<a:t>` 정규식 `.`→`[\s\S]` 버그(개행 포함 런 누락) 동시 수정. 검증 `scripts/test-pptx-table.mjs`(20슬라이드, 3×5 표 보존, +2.5ms). 병합 셀은 마크다운 한계로 근사. → **표는 모든 텍스트 추출 포맷(HWP/DOCX/XLSX/CSV/PPTX) 보존**. **+업로드 가이드** [REF_Document.md](guide/REF_Document.md) 신규(전 타입·PDF 네이티브 흐름·search-gate). **+generator.ts 리팩토링 1·2·4-A순위(§10, move-only·동작 불변)**: 1148줄 단일 함수에서 closure 비의존 순수함수 3모듈(`pill-messages`·`sdk-contents`·`generation-config`) + 검색게이트(`search-gate` `decideGoogleSearch`) + 에러분류(`retry` `isTimeoutError`/`isAuthError`/`markRateLimitKey`, SDK·LangChain catch의 byte-identical 중복분만) 분리. 단계별 tsc 0 + 동등성 테스트(`scripts/test-generator-refactor.mts`, 추출 vs 인라인 deep-equal) **19,423 케이스 0 fail**. generator.ts 1148→883줄(-23%). **3-A(LangChain 경로 분리)·drug_info 버그픽스는 익일 [DEV_260622](logs/DEV_260622.md)**. 로그 [DEV_260621 §6·§7·§8·§9·§10](logs/DEV_260621.md)·계획 [PLAN_GENERATOR_REFACTOR_260621](plans/PLAN_GENERATOR_REFACTOR_260621.md).
- [DEV_260620.md](logs/DEV_260620.md) — **docs 정합성 정리 + kordoc 한글문서 파싱 연동 검토·검증·계획 + 이미지+검색 할루시네이션 수정 + dev 서버 경고 해소**: ① docs staleness 교정 — TODO.md(완료된 영화 ④ 블록 제거·FontAwesome 전제 역전 정정·신규 툴 경로 `server/agent/tools/`→`server/agent/`·Lighthouse·maxOutputTokens 라인 드리프트), PLAN_INDEX(존재하지 않는 "TODO.md P0" 참조 제거, 보안은 백로그 §보안/SSRF는 DEV_260504 완료), DEV_HISTORY(v4.78부터 일별 로그 링크 형식임을 안내 노트로 명확화). ② **kordoc(v3.1.1) 연동** — `.hwpx`만 JSZip `<hp:t>`(표 소실)·구형 `.hwp`/`.hwp3`/`.hwpml` 미지원인 현 상태를 서버 라우트로 개선. **스코프: HWP 4종만 kordoc, PDF는 native 유지(멀티모달 회귀 방지)**. 검증 스크립트 4종 작성, 정부 사업계획서 4종(문체부 3.64MB·중기부 3.91MB·데이터바우처 1.71MB·NIA 32MB) 실측 — **전부 파싱 성공·표 `<table>` 구조 보존**, 기존 방식 대비 셀 경계 보존이 핵심 가치. **🔴 전송 재설계**: 3/4가 base64 시 Vercel 4.5MB 초과 → **4MB 임계값 라우팅 확정**(raw≤4MB 직행 multipart=55~62% 빠름 / 초과는 Storage 경유 signed-url). 첨부 시점 백그라운드 파싱으로 체감 제거. 계획서 `docs/plans/PLAN_KORDOC_INTEGRATION_260620.md` 수립·PLAN_INDEX P1 등재. ⚠️ kordoc 신규 취약점 12건 → 배포 전 점검. ③ **이미지+검색 할루시네이션 수정(구현 완료)** — 이미지 첨부 후 "검색해서 확인" 후속턴이 grounding 없이 가짜 출처(`[1]`·ScienceDaily URL) 생성하던 버그 근본 수정. **`generator.ts` `dropImageForSearch` 가드 신설**: `intent=general` && 현재 턴 이미지 없음 && history 이미지 있음 && `_explicitSearchForGuard` 조건 충족 시 `forceTextOnly=true`(history 이미지 파트 제거) + `useGoogleSearch` 강제 허용 → 실제 grounding 수행 → 진짜 소스 칩 표시. **`_explicitSearchForGuard` 구조 개선**: 초기 키워드 정규식 단독 → `state.needsSearch === true`(라우터 LLM 문맥 판정) 1순위 + 키워드 정규식 보조로 변경 — "팩트체크를 위해 웹에서 검토해보자" 같이 키워드 없는 의미상 검색 요청을 라우터가 이미 `needsSearch=true`로 판정했음에도 제너레이터가 무시하던 구조적 불일치 해소. **`prompt.ts` 절대 규칙 삽입**: `[CRITICAL — ABSOLUTE RULE: NEVER FABRICATE SOURCES]` 블록을 모든 포맷 지시보다 최우선 위치에 배치 — `google_search` 실제 호출 시에만 인용 허용, 미호출 시 `[N]`·참고 자료·URL·"검색 결과" 표현 일절 금지. 모델명 `Gemini 2.5 Flash` → `Gemini 3.5 Flash`. ④ **`next.config.ts` `allowedDevOrigins: ['127.0.0.1']`** — dev 서버 HMR cross-origin 경고 해소(프로덕션 영향 없음). (kordoc 구현은 DEV_260621 §6·§7에서 완료)
