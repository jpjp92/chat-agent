...
- [DEV_260514.md](DEV_260514.md) — **URL 수집 Jina Reader fallback 적용**: Wikidocs 블로그 URL 요약 실패 원인을 테스트 스크립트로 확인. Node 서버 사이드 직접 fetch는 `403 Forbidden` + `Just a moment...` Cloudflare challenge HTML만 수신해 selector/Cheerio 보강으로는 해결 불가. `scripts/test-url-extract.mjs`로 직접 fetch·기존 추출·Wikidocs selector·Cheerio 후보·Jina Reader 후보를 비교했고, Jina Reader는 `200 OK`와 실제 본문 Markdown 약 4.4KB를 확보. `api/fetch-url.ts`에 challenge 감지(`403`, `Just a moment...`, `cf-chl`, `cloudflare`, `challenge-platform`)와 Jina Reader fallback을 추가. 직접 수집이 짧을 때는 Jina를 보조 시도하되 실패하면 기존 짧은 본문을 유지. 사용자-facing 원문 직접 수집 실패 안내는 추가하지 않음. `npx tsc --noEmit` 성공. / **Jina Reader URL 이중 프로토콜 버그 수정**: 네이버 뉴스 등 URL 요약 시 실제 기사와 전혀 다른 내용이 반환되는 증상 확인. `fetchJinaReaderContent`에서 Jina Reader URL을 `https://r.jina.ai/http://${targetUrl}` 형태로 생성했으나, `targetUrl`에 이미 `https://`가 포함되어 있어 이중 프로토콜(`http://https://...`) 잘못된 URL이 전달됨. Jina Reader가 이를 해석하지 못하고 캐시된 무관한 기사를 반환하는 것이 원인. `https://r.jina.ai/${targetUrl}` 로 수정. / **isBlockedOrChallenge 오탐 수정 및 Jina fallback 우선순위 재설계**: 이중 프로토콜 버그 수정 후에도 완전히 다른 기사가 반환되는 케이스 지속. 원인 1 — `isBlockedOrChallenge`의 `lower.includes('cloudflare')` 단순 체크가 CDN 사용 정상 사이트에서 오탐, `lower.includes('just a moment')`도 본문 포함 시 오탐 가능 → `<title>` 한정 체크 + `cloudflare && checking your browser` AND 조합으로 축소. 원인 2 — `content.length < 300` 조건으로 직접 fetch 성공 후에도 Jina를 추가 호출하는 경로가 존재, Jina가 캐시된 엉뚱한 기사를 반환하면 그대로 덮어씌워짐 → 해당 경로 제거. 현재 동작: 직접 fetch 성공(길이 무관) → 그대로 사용 / 직접 fetch 완전 실패 → Jina fallback / Jina도 실패 → `[FETCH_ERROR]` → Google Search.
...
- [DEV_260426.md](DEV_260426.md) — **스트리밍 중 `**` 볼드 마커 dangling 수정** / **날씨 이모지 테이블 누락 수정** / **MFDS 미등재 약품 검색 폴백 개선** / **MFDS 폴백 출처 칩 미표시 수정 3단계** / **소스 칩 스트리밍 완료 후 표시** / **첨부파일 UX 전면 개선** (자세한 내용은 DEV_260426.md처럼 수정 1~4 참조) / **이미지 썸네일 aspect-ratio 16/9 컨테이너** (`max-w-[220px]`, 폴백 컨테이너 크기 고정, 아이콘 축소) / **이미지 항상 Supabase 업로드** (크기 무관 `chat-imgs` 버킷 업로드 후 URL DB 저장 → 히스토리 미리보기 복원, `useChatStream.ts`)
