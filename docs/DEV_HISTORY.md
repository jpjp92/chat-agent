...
- [DEV_260522.md](DEV_260522.md) — **3.5 Flash 멀티턴 포맷 보정 3종 + 볼드 렌더링 수정**: ① **Two-track 고정 포맷 반복**: Stage2 `synthesisInstruction`에 `isMultiTurn = sdkContents.length > 1` 분기 추가 — 1턴은 "한 줄 요약 → 주요 내용 → 고려 사항" 3단 구조 유지, 2턴 이상은 고정 섹션 없이 자연스러운 응답 지시로 전환. ② **PDF/URL 멀티턴 two-track 오분기**: `historyHasUrl` 히스토리 스캔 추가 — 이전 사용자 메시지에 URL이 있으면 `useGoogleSearch = false` 강제. ③ **Stage2 포맷 요청 무시**: `synthesisInstruction` 최상단에 `[USER_REQUEST]` 섹션 추가. `scripts/test-multiturn-35flash.mjs` 사전 검증 완료. ④ **`**볼드**` 렌더링 실패**: CommonMark 규칙상 닫는 `**` 뒤에 글자(한국어 조사, 영어 복수형 등)가 바로 오면 closer로 인정되지 않음 → `components/ChatMessage.tsx` 텍스트 전처리 3곳에 `/([a-zA-Z가-힣\d])\*\*([가-힣A-Za-zÀ-ÿ])/g` 추가해 공백 삽입. 한국어·영어·프랑스어·스페인어 14개 케이스 `scripts/test-bold-rendering.mjs`로 검증.
