...
- [DEV_260426.md](DEV_260426.md) — **스트리밍 중 `**` 볼드 마커 dangling 수정** (홀수 `**` 감지 시 닫기 추가, backtick 처리와 동일 패턴, `ChatMessage.tsx` 2곳) / **날씨 이모지 테이블 누락 수정** (예시-가이드 이모지 불일치 해소, 테이블 셀 이모지 MANDATORY 지시 추가, `prompt.ts`) / **MFDS 미등재 약품 검색 폴백 개선** (파스·연고 등 비알약 제형 — Google Search grounding → DuckDuckGo → LLM 내부 지식 3단계 폴백 체인, `searchDrugViaGoogleSearch()` 헬퍼 추가, 시스템 프롬프트 예외 명시, 스트리밍 누출 방어) / **MFDS 폴백 출처 칩 미표시 수정** (`groundingChunks` → `webSearchQueries` 폴백, `on_chain_end` LangGraph ToolMessage 직접 스캔, `pendingSources`로 타이밍 해결) / **소스 칩 스트리밍 완료 후 표시** (`sources` 콜백 즉시 반영 제거 → `finally` 일괄 적용, `useChatStream.ts`) / **첨부파일 타입 레이블 간소화** (이미지 `IMG`, PDF 카드 스타일 통일, 문서 확장자 뱃지 추가, `ChatMessage.tsx`) / **첨부파일 깨진 이미지 아이콘 대체** (`AttachmentImage` 컴포넌트 추가, `onError` 시 fa-image 폴백, `ChatMessage.tsx`) / **첨부파일 mimeType·fileName 오추론 수정** (`EXT_MIME` 맵 + `inferAttachment` 확장자 우선 추론, `useChatSessions.ts`) / **첨부파일 아이콘 색상 버그 수정** (`text-slate-400` 분리, hwpx 체크 추가) / **스트리밍 `[N]` citation 청크 분할 수정** (lookbehind citationBuffer 패턴으로 청크 경계 strip 누락 해소, `generator.ts`·`chat.ts`) / **첨부파일 UX 개선 — 직사각형 레이아웃 & 문서 카드 정보 밀도** (이미지·비디오 `128×72px` / 문서 `160×72px` 직사각형 썸네일 통일, hover 오버레이 삭제 버튼, 채팅 이력 이미지 `h-200px` 고정, 문서 카드 아이콘 `w-12 h-12` · `text-2xl`, 파일명 `max-w-[280px]`, 파일 크기 `· 2.4 MB` 표시, 타입별 hover 색상, `fileSize?: number` 타입 추가, `formatFileSize()` 헬퍼, `AttachmentImage` `style` prop 추가)
