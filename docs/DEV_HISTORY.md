...
- [DEV_260529.md](logs/DEV_260529.md) — **이미지 의도 분류 버그 수정**: 이미지 첨부 시 약품 무관 케이스가 모두 `drug_id` → Vision Node로 강제 라우팅되던 문제. `isAmbiguousImageIdentificationRequest()` 술어가 빈 캡션/일반 표현에 `true` 반환 → `hasMedicalIntentKeyword`(실제 의료 키워드)로 교체. / **약품 식별 DB 조회 개선**: ① `identifyPillTool.invoke()` ToolMessage 객체 반환 시 `"[object Object]"` 변환으로 fast-pass 실패 → `.content` 추출로 수정 ② Vision 프롬프트 `"dHP"` 예시 제거(편향 원인) + score line vs 각인 구분 명시 ③ `filterResults`에 1단계-b(각인 완전 일치·색상 불일치 허용) 추가, 2단계-b 변형 매칭을 2자 전용으로 제한 ④ pharm.or.kr 검색 대소문자 보존(`"dP"`) 쿼리 추가 및 3자→2자 역방향 검색 추가. / **pharm.or.kr 구조적 한계 확인**: 마그네스정이 각인 텍스트 없이 이미지로만 저장되어 각인 검색 불가. 색상/모양 파라미터도 동작 안 함. / **MFDS Open API 조사** (`scripts/test-mfds-pill-api.mjs`): `MARK_CODE_BACK_ANAL: "dHP,dP,d-P,cHP"` — 모든 각인 변형 저장 확인. 단 `item_name` 외 필터 파라미터 전부 무시(25,558건 = 전체 DB 고정 반환). / **다음 단계**: Supabase `mfds_pills` 테이블 추가 + 전체 25,558건 동기화 계획. `mark_codes TEXT[]` + GIN 인덱스로 `mark_codes @> ARRAY['DP']` 쿼리. 동기화 스크립트 `scripts/sync-mfds-pills.mjs` 작성 완료.
