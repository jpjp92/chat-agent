...
- [DEV_260605.md](logs/DEV_260605.md) — **Wikidocs Playwright fallback 실제 대화 플로우 검증**: `https://wikidocs.net/blog/@jaehong/10121/` 실제 UI 대화에서 `/api/fetch-url` Playwright fallback 성공(`article`, 4,964자, 3.7s) → `[URL_CONTENT]` 주입 → `/api/chat`에서 `URL content provided — Google Search disabled` 확인 → Gemini 3.5 Flash가 검색 없이 제공 원문 기반 응답 경로 진입. 남은 확인은 Vercel Preview/Production에서 `@sparticuz/chromium` 실행, function size, cold start 로그.
