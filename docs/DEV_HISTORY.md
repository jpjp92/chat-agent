...
- [DEV_260511.md](DEV_260511.md) — **Vet-Viz 전국 동물병원 검색 구현** (행정안전부 `1741000/animal_hospitals/info` API, `returnType=JSON` 직접 수신, `cond[ROAD_NM_ADDR::LIKE]` 텍스트 주소 검색, 영업중 필터 0건 시 전체 재조회, EPSG:5174 좌표 WGS84 불가 → 카카오지도 텍스트 검색, VetRenderer 틸 테마·🐾·영업상태 배지) + **버그 3종**: `LANGCHAIN_INTENTS` `vet_search` 누락(SDK 경로 오분기) / serviceKey URLSearchParams 이중 인코딩 → Unauthorized / `chat.ts` `on_tool_end` vetTool 핸들러 누락 → empty response 에러 + **에러 보완 5종**: 검색 조건 없는 전국 첫 페이지 노출 차단 / VET_KEY 누락 가드 / HTTP·JSON·API 오류 분리 / 병원명 단독 검색 fallback 확장 / `펫` 키워드 과매칭 축소
