...
⚠️ **HWP 5.x 바이너리(`.hwp`)·`.hwp3`·`.hwpml` 은 실제 파일로 확인하지 못했다** — 레포에 픽스처가 없고 합성이 어렵다. HWPX 와 실패 경로만 실측이다. **dev 배포 후 실제 `.hwp` 첨부 1건이 남은 검증이다.**
...
| # | 작업 | 우선순위 |
|---|---|---|
| 1 | **dev 배포 후 실제 `.hwp` 첨부 1건** — kordoc major 의 유일한 미검증 경로(§3) | 🔴 높음 |
