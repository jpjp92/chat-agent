...
> ⚠️ 이 부류가 프로덕션에서 일반 검색으로 새던 케이스(DEV_260612). 라우터 LLM 실패 시 정규식 폴백이 받도록 `intent-rules.ts` 패턴을 보강함.
>
...
> **동작 원리**: `MovieRenderer`가 가져온 상영표를 클라 스토어(`lib/movie-context.ts`)에 적재 → 다음 메시지에 `movieContext`로 동봉 → 라우터가 "카드 존재 + 질문형"이면 `movie_search`가 아니라 `general`로 보내 화면 데이터로 답하게 함(검색 off로 일반 표 폴백 차단). 데이터에 없는 정보(줄거리·평점·예매율·다른 지역)는 지어내지 않고 검색/재조회로 안내.
...
3사 기본 지점·드롭다운 목록은 `data/theater-branches.json`(424지점)에서 온다. **런타임 fetch가 아니라 [lib/theaters.ts](../../lib/theaters.ts)가 빌드 타임에 번들로 import** — 0ms·네트워크 없음. DB로 옮기지 않는 이유와 정책: [PLAN_THEATER_BRANCHES_DATA_260627.md](../plans/PLAN_THEATER_BRANCHES_DATA_260627.md).

데이터는 커밋되어 있고, 갱신은 **로컬 전용 스크립트**(gitignore `scripts/test-*`, 레포 미포함)로 수동 실행:

```bash
node scripts/test-branch-list.mjs   # 로컬 전용 — data/theater-branches.json 갱신
```

- **언제**: 분기 1회, 또는 카드 드롭다운에서 지점이 빠졌다는 신고 시.
- **요구**: `BROWSERLESS_KEY`(CGV browserless 스크랩). 롯데·메가박스는 direct.
- **안전장치**: 스크랩 결과가 임계치 미만(`CGV<150`/`롯데<115`/`메가<100`)이면 **기존 파일을 덮지 않고 exit 1**(붕괴 방지). 통과 시 tmp→rename 원자적 교체.
- **정규화(생성 시)**: HTML 엔티티 디코드 + 쓰레기(`href`·빈 값)·영구 폐점(`영업종료`) 제거. `휴관`은 라벨 유지. 롯데 DivisionCode 2(중복) 제거.
