...
```
(카드 표시 후) 부산은?   /   제주도 날씨도   /   내일 부산 비와?
```
→ **화면에 없는 도시**가 지목됨 → **weather** 재조회, 새 카드.

> ⚠️ **2026-08-17 동작 변경**: 시점 이동(`내일은?`·`이번 주말 날씨 어때?`)은 더 이상 재조회가 아니라
> **§4의 설명 경로**로 간다. 카드가 이미 +5일 예보를 담고 있어 재조회해도 **같은 데이터**가 오고,
> 그동안은 새 카드만 뜨고 질문에 답하지 않았다("내일 비와?"에 카드만 세 번 뜬 실측).
> 판정 축이 *"날씨를 물었나"* 에서 **"화면에 없는 도시가 나왔나"** 로 바뀌었다.
...
- ⚠️ **weather 규칙은 `날씨`·`기온` 단독으로는 안 잡는다** (2026-08-16). 이 규칙은 폴백이자 **구제 경로**라 라우터 LLM이 성공해도 검사되는데, `날씨 좋은 날 뭐하지`·`분위기가 기온처럼 오르내려`처럼 배경·비유로 쓰인 발화에까지 카드가 떴다. → 질의어 동반(`어때`·`알려`·`예보`·`몇 도`…) 또는 **문장 끝**(`부산 날씨`)일 때만. 토큰을 추가할 땐 `npx tsx tests/test-intent-rules.mts`를 함께 돌릴 것 — 좁히기 과잉으로 정당한 질의를 놓치는 반대 방향 회귀가 이 규칙에서 가장 나기 쉽다.
- ⚠️ **후속 판정은 `server/agent/weather-followup.ts`의 순수 함수**다(2026-08-17에 라우터 인라인에서 분리).
  하니스가 임포트해야 해서 뺐다 — `npx tsx tests/test-weather-followup.mts`.
  판정 축은 **"화면에 없는 도시가 나왔나"** 하나다. 예전 `날씨 + 알려/줘 → 새 조회` 규칙은 삭제했다:
  날씨를 묻는 **모든** 자연스러운 발화에 걸려 `부산 날씨 알려줘`와 `날씨 추세 알려줘`를 구분하지 못했고,
  그 결과 카드만 반복 생성되고 질문에는 한 번도 답하지 않았다.
  · 도시 사전은 `server/lib/weather`의 `isKnownCityName`(= `CITY_ALIASES` 재사용, **복사 금지**).
  · `날씨` 앞 토큰의 거부목록은 **닫힌 부류만**(지시어·담화표지·시간어). 도시명은 열린 부류라
    거부목록으로 못 다룬다 — `아니`가 없어서 "아니 날씨 추세 알려줘"의 `아니`가 도시로 판정됐었다.
  · **남은 한계**: 사전에 없는 지명을 `날씨` 없이 말하면(`순천 비와?`) 새 조회로 안 잡혀 **떠 있는 카드로 답한다.**
    사전에 도시를 추가하는 것이 정공법이다.
- ⚠️ **후속 턴에는 날짜 대응표를 서버가 만들어 준다**(`buildDateLadder`, 2026-08-17). `[CURRENT_SYSTEM_TIME]`이
  이미 있어도 `daily[].date`와 "내일"의 대응은 **별개의 계산**이라 모델이 틀린다 — 실측에서 `내일 서울 비와?`에
  **오늘** 강수(`19mm·60%`)를 답하고 같은 답변에서 18일을 "모레"라 불렀다(하루씩 밀림).
  · 표는 `오늘/내일/모레/글피 = YYYY-MM-DD(요일)` 4줄 + "`current`·히어로 블록은 오늘 값이니 미래 날짜엔 쓰지 말 것".
  · 표 자체는 결정론이라 하니스가 검증한다(월말·연말·**KST 자정 경계**).
  · 🔴 **표가 맞다고 답변이 맞는다는 보장은 없다** — 산문 정확도는 실사용 확인이 유일한 판정이다.
