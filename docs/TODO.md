...
- [x] **v4.11: YouTube In-Chat Playback & Freeze Fix**: `YoutubeEmbed` 컴포넌트 추가 및 태그 정합성 수정을 통해 30분 이상의 긴 영상 분석 안정성 확보.
...
- [x] **v4.11: 토스트 알림 및 로그 정리**: 프로필 수정, 세션 이름 변경 등 반복적인 Toast 메시지를 제거하여 UI 노이즈를 줄이고, 불필요한 디버그 로그(`console.log`)를 정리하여 production 환경 최적화. ✅
...
_Last Updated: 2026-03-10 (YouTube 인챗 플레이어 추가, 긴 영상 프리징 해결 및 UI 노이즈 제거)_
