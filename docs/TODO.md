...
### 4. `src/` 이주 완결 — **방향은 정해졌고 실행만 남았다**

레이아웃이 두 벌이다. Vite 시절 루트 배치(`App.tsx`·`components/`·`services/`·`utils/`·`lib/`)와
`src/` 로의 이주가 **4개 파일에서 멈춘 채** 공존한다.

```
루트   App.tsx · components/ 26 · services/ 1 · utils/ 3 · lib/ 7 · app/ 15
src/   src/hooks/ 3 · src/lib/ 1        ← 여기서 멈췄다
```

**결정(2026-09-24): `src/` 4개를 루트로 되돌린다.** 반대 방향(전부 `src/` 로)보다 싸고,
이 레포의 다수파에 맞춘다. Next.js 는 둘 다 지원하므로 기능 제약은 없다.

| 방향 | 걸리는 참조 |
|---|---:|
| **`src/` → 루트** (채택) | 문서 129건 + import 14건 |
| 루트 → `src/` | 문서 1,071건(`components/` 261 · `lib/` 378 · `server/agent/` 432) + import 다수 |

- [ ] `src/lib/models.ts` → `lib/models.ts` (import 11곳)
- [ ] `src/hooks/` → `hooks/` (import 3곳, 파일명 `useXxx.ts` 는 유지 — React 관례)
- [ ] 문서 경로 129건 갱신 — `test-doc-links` 가 전수로 잡는다

> ⚠️ **프롬프트 계층화 8·9단계와 같은 PR 에 넣지 않는다.** 그 단계들은 응답이 바뀔 수 있는
> 구간이라([PLAN_PROMPT_LAYERING §10](plans/PLAN_PROMPT_LAYERING_260923.md)), 대규모 파일 이동이
> 겹치면 회귀 원인을 못 가린다. 이름·경로만 바꾸는 독립 커밋으로 보낸다.
>
> 🔴 **미루면 비용이 오른다** — `src/` 에 파일이 하나 붙을 때마다 참조가 늘어난다.
> 케이싱 불일치 9건 중 4건이 정확히 `src/` 와 옛 `utils/`·`services/` 에 있었다
> ([컨벤션](guide/REF_Architecture.md#코드-컨벤션)).

### 5. 프롬프트 계층 충돌 프로브 — 장애 턴에서 두 계층이 반대로 말한다

PubMed 장애 턴(`error` + `count: 0`)을 조립해 보니 한 프롬프트에 **792자 간격**으로 둘 다 실린다:
...
정면 모순은 아니다("일반 지식 제공" ≠ "근거 판정"). 그러나 턴 규칙은 **약국·법령용으로 쓰였고**
("울릉도 약국"에 아는 걸 말해주는 건 무해하다), 그게 지금 **의학 근거 질문에도 발동한다.**

- [x] 프로브 신설·측정 완료 (2026-09-25, **4팔 × 7회 = 28회**) — [`probe-layer-conflict.mts`](../tests/manual/probe-layer-conflict.mts)

🔴 **가설은 기각(0/7), 그런데 다른 게 깨져 있었다.** 모델은 장애를 7/7 밝혔고 "근거 없음"으로
바꾸지 않았다. [DEV_260830 §6.29](logs/2026/08/DEV_260830.md) 의 **세 원인**을 갈라 재니:

| 원인 | 예 | 통과 | 판단 |
|---|---|---:|---|
| ③ 백엔드 장애 | PubMed 500 | **7/7** | 안 고친다 — 설계표대로 "확인 못 했다"고 밝힌다 |
| ② 조회했는데 0건 | 진짜 논문 질문 | **7/7** | 안 고친다 — "다음 수 안내"가 없어 반쪽이지만 오답은 아니다 |
| **① 오분류** | `불면증에 도움되는 방법 연구된 거` | **0/7** | ~~고친다~~ → **결함 아님(아래 종결)** |

①에 대해 *"`강아지 사료` 와 같은 등급"* 이라고 썼는데 **과했다**(아래 종결 참조 — 그때는 원하는
것에 닿는 경로가 아예 없었고, 여기는 `medical_qa` 가 있다). 라우터는 `paper_search` 로
보내고(7/7), PubMed 0건이면 **7회 전부** 이렇게 끝난다:

> 불면증에 도움이 되는 방법에 대한 연구는 찾을 수 없었습니다. / 개인적인 건강 문제에 대해서는
> 항상 의료 전문가와 상담하는 것이 중요합니다.

수면위생·카페인 회피는 안전하고 기본적인 지식인데 하나도 주지 않는다.
당시엔 `buildEmptyCardRules` 가 막으려던 결함이 **논문 의도에서만 안 막힌다**고 읽었다.

#### 🔴 종결 — ①도 결함이 아니었다 (2026-09-25, 100회 측정)

위 ① 판정은 **라우터를 안 보고** 내렸다. 수정을 넣어 재보니 **0/7 → 3/7** 밖에 안 올라
원인을 캐다가 알았다:

| 시험 문구 | 라우터 | eutils |
|---|---|---:|
| `비타민D 결핍이면 어떤 증상이 생겨? 관련 연구도 같이` | **`medical_qa`** | **0** |
| `불면증 관련 연구 알려줘. 그리고 당장 뭘 해보면 좋을지도` | **`medical_qa`** | **0** |

**답이 주목적인 질문은 `paper_search` 에 닿지 않는다** — `router.ts` 가 그렇게 정의한다.
논문 도구는 호출조차 안 되고 `medical_qa` 가 1만자 답변을 준다. `paper_search` 에 닿는 것은
**문헌 자체를 달라는 요청뿐**이고, 그때 *"연구를 찾을 수 없습니다"* 는 설계표가 정한 맞는 답이다.

남는 노출은 C(`불면증에 도움되는 방법 연구된 거 알려줘`) 처럼 **한 문장이 두 갈래로 읽히는
좁은 회색지대**뿐이다. 사람도 갈리는 문장이라 3/7 을 결함으로 부르기 어렵다.

- [x] **수정을 되돌렸다** — +751자를 주고 3/7 을 얻는 규칙은 프롬프트 군살이고, "고쳤다"고
  남기면 다음 사람이 처리된 문제로 읽는다. 상세: [DEV_260925 §8](logs/2026/09/DEV_260925.md)
- [x] **프로브는 남겼다** — `reachedPaperPath()`(논문 경로 미도달 턴은 ⚪ 무효) · `--only` 필터.
  🔴 없었으면 `medical_qa` 의 정상 인용 마커를 **7/7 결함으로** 오판했다(판정기 결함 3건째)
- [ ] **②의 "다음 수 안내"**(더 넓은 검색어·arXiv 제안) — 이것만 남는다. 오답이 아니라 품질 손실

> **배운 것**: 프롬프트 규칙이 지켜지는지 재기 전에 **그 프롬프트에 입력이 닿는지**를 먼저 잰다.
> §10-7(프롬프트 본문만 보고 세운 가설)에 이어 **두 번째로 같은 실수**를 했다.
