...
| **① 오분류** | `불면증에 도움되는 방법 연구된 거` | **0/7** 🔴 | **고친다** |

①은 `강아지 사료 추천해줘` → 동물병원 빈 카드와 **같은 등급**이다. 라우터는 `paper_search` 로
...
`buildEmptyCardRules` 가 막으려고 만들어진 결함이 **논문 의도에서만 안 막힌다.**

- [ ] **조치 결정** — 후보: 논문 정책의 0건 조항에 *"사용자가 물은 것이 논문 목록이 아니면
  그 질문에 답하라"* 를 더한다. 턴 규칙을 건드리는 것보다 좁다. ⚠️ **응답이 바뀌므로 전후
  7회씩 측정을 동반한다**(같은 프로브로 A/B 가능)
- [ ] ②의 "다음 수 안내"(더 넓은 검색어·arXiv)는 별건으로 둔다 — 같은 수정에 섞으면 원인이 섞인다

> 🔴 라우터는 **극단은 막는다** — `논문 쓰는 법 알려줘` 는 `general` 로 가 도구를 호출조차
> 안 했다(1,647자 정답). 노출 구간은 **"연구/논문" 낱말이 붙었지만 원한 건 방법·설명인 질문**으로
> 좁다. 라우터가 틀렸다고 하기 어려운 회색지대라 **라우터가 아니라 0건 응답 방식**이 고칠 자리다.
