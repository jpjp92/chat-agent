...
| 13 | Next.js `16.2.11+` 로 올릴 것 | ⚖️ **이행됨 (2026-09-13)** — 16.2.6 → **16.3.5** | [DEV_260913_DEPS](../logs/2026/09/DEV_260913_DEPS.md) · §2-D |
...
### 2-D. Next.js 버전 — ⚖️ 2026-09-13 이행됨

> **이 절의 유보는 해소됐다.** 레지스트리를 확인할 수 있게 되자 advisory 범위는
> `16.2.11` 이 아니라 **`9.3.4-canary.0 - 16.3.2`** 였다 — **16.2.11 로 올렸다면 여전히 취약했다.**
> `next` 16.2.6 → **16.3.5**, `eslint-config-next` 도 16.3.5 로 맞췄다. Critical 2 + High 4 + Moderate 5 가 닫혔다.
> 검증·도달 가능성 분석은 [DEV_260913_DEPS](../logs/2026/09/DEV_260913_DEPS.md).
> 📌 **이 절이 유보한 판단은 옳았다** — "16.2.11 에서 수정"은 확인 안 한 전언이었고 **실제로 틀린 값이었다.**
...
> **2026-09-06 모델 도입과의 관계:** [Gemini 3.8 계획](PLAN_MODEL_3_8_MIGRATION_260906.md)의 로컬 검증은 함께 준비하되, 선택 옵션 공개는 아래 1·2 완료 후로 둔다. SSRF·원자적 쿼터 작업의 우선순위는 유지한다.
