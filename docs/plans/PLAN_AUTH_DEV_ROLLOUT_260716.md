...
### C′. 로컬 선검증 (머지 전, 스테이징 DB 바인딩) — 진행 중 (2026-07-19)
- [x] **시나리오 1 — 게스트→Google 승계** 통과 (같은 uuid·대화 보존·이름/아바타 동기화). 상세 [DEV_260719](../logs/2026/07/DEV_260719.md) §1
- [x] 검증 중 버그 2개 발견·수정([DEV_260719](../logs/2026/07/DEV_260719.md)): 로그아웃 "연결 실패" 번쩍임 · 충돌 재시도 계정 재선택 — 2파일(useAuthSession·App), tsc 0
- [ ] 시나리오 2 — 대화 0개 게스트 → 바로 로그인(충돌 없이)
- [ ] 시나리오 3 — 파일 업로드 → 스테이징 버킷 저장
- 준비 메모: 스테이징 잔여 정식 유저가 Google 계정을 소진해 승계가 계속 충돌 → `johnnyworld9278`(메시지 0) 1명 정리 후 확인.

### D. 브랜치 머지 (담당: 사용자 — 커밋)
- [ ] 로컬 선검증(C′) 완료 후 `feat/auth-mvp` → dev 머지 (코드 무변경)
