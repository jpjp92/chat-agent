...
| 8 | 조건부 블록을 소스 축으로 이동 (§7-2) — **줄 단위 분해** 🟡 **2026-09-25 영상·SOURCE 완료, REFORMAT 잔여(§10-10)** | 중~높음 | 블록 통째 이동 금지. 전역 줄은 남기고 조건부 줄만 옮긴다 · 영상·URL 턴 회귀 측정 동반 → **§10-9** |
...
> 🔴 **2026-09-25 정정 — 이 가설은 기각됐다(0/7).** 프로브를 돌린 결과 모델은 장애를
> 7/7 밝혔고 "근거 없음" 판정으로 바꾸지 않았다. **틀린 쌍을 보고 있었다** — 턴 규칙과
> 부딪히는 건 장애 조항이 아니라 `paper_search` 의 **분량 조항**(*"answer in ONE short
> paragraph"*)이고, 그래서 **진짜 0건 턴에서도 똑같이** 일어난다. 대신 반대편이 깨져 있었다:
> 턴 규칙의 *"아는 내용을 반드시 함께 주세요"* 가 **0/7 지켜졌다**(사용자는 61자를 받는다).
> [DEV_260925](../logs/2026/09/DEV_260925.md) · [프로브](../../tests/manual/probe-layer-conflict.mts)
>
> 이 절을 지우지 않고 남긴다 — **프롬프트 내용만 보고 응답을 예측하면 틀린다**는 사례라서다.
>
> 🔴 **2026-09-25 2차 정정 — 위 "0/7 깨졌다"도 결함이 아니었다.** 0건 조항에 분기를 더해
> 재보니 **3/7** 밖에 안 올랐고, 원인을 캐니 **라우터가 이미 갈라내고 있었다**: 답이 주목적인
> 질문(`비타민D 결핍 증상 + 관련 연구`, `불면증 연구 + 뭘 해볼지`)은 두 문구 모두
> **`medical_qa` 로 가 논문 도구를 호출조차 하지 않고** 1만자 답변을 준다. `paper_search` 에
> 닿는 것은 문헌 자체를 달라는 요청뿐이고, 그때 짧은 "없습니다"는 설계표대로다.
> **수정을 되돌렸다.** [DEV_260925 §8](../logs/2026/09/DEV_260925.md)
>
> 같은 실수를 **두 번** 했다: 1차는 프롬프트 본문만 보고, 2차는 라우터를 안 보고.
> 교훈은 하나다 — **프롬프트 규칙이 지켜지는지 재기 전에 그 프롬프트에 입력이 닿는지를 먼저 잰다.**
...
### 10-10. 8단계 2차 (2026-09-25) — `SOURCE ADHERENCE` 를 줄 단위로

§7-2 가 센 *"조건부 3,323 / 전역 2,070"* 이 맞았다. **통째 이동 금지가 옳았던 이유**는
`[ONE-LINE SUMMARY FORMAT]`(전역)이 URL 3단 템플릿(조건부)과 `[CSV/XLSX]`(조건부) **사이에
끼어 있기** 때문이다. 블록으로 옮기면 전역 규칙이 조건부가 된다.

게이트 둘(`sourceTurn` = `!!state.webContent`, `videoTurn`)로 갈랐다. **자리는 그대로** —
본문·영상이 있는 턴은 **바이트가 동일**하고, 일반 대화 턴은 base 의 **27%**(5,799자)를 덜 싣는다.
의도 골든 25개가 **전부 정확히 2,848자씩** 줄었다(증분이 하나뿐 = base 게이팅 외 무변화).

🔴 **자수 골든은 줄 재배치를 못 잡는다.** M4(줄 하나를 아래로 이동, 자수 동일)에 114개가
전부 통과했다 — sha 골든이 모두 `webContent: ''` 여서 `sourceTurn=true` base 를 아무도 안 봤다.
`BASE_GOLDEN` 을 **4조합 × 4언어 = 16개를 `자수/sha`** 로 박아 메웠고, 게이트가 **줄을 빼기만
한다**는 불변식(줄 부분수열)도 검사로 넣었다.

상세·판정 근거: [DEV_260925 §11](../logs/2026/09/DEV_260925.md)
