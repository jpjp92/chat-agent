...
## 5. 권장 — 계층적 + "공유 vs 렌더러-로컬" 경계 (적용 대기)

| 단계 | 작업 | 리스크 |
|---|---|---|
| **1** | 죽은 키 6개(+타입만 있는 2개) 삭제 | 0 (순수 삭제) |
| **2** | statusMessages 배관 제거 → useChatStream로, `language`만 전달 | 아주 낮음 (검증: `npx tsc --noEmit` + 로딩텍스트 스모크) |
| **3** | `src/i18n/`에 **공통 문자열만**(에러·status·푸터·공용 UI) 타입드 모듈 + 얇은 `t(language,key)` | 낮음 |
| **4** | 렌더러 i18n은 **각 렌더러 안에 유지** (lazy 청크 보존) | — |
| **5 (보류)** | Drug(9삼항)·App(5삼항) worst-first 점진 이관, 나머지는 건드릴 때만 | — |
