...
> 작성일: 2026-05-30
> 상태: 구현 완료. **일부 폐기됨** — 아래 ⚠️ 참조
> 기준 모델: Gemini 3.5 Flash (기본값)
> 제약: 기존 13종 intent 분류 및 도구 로직을 깨지 않는다.

> ### ⚠️ 2026-08-15 — 이 문서의 일부는 [PLAN_SEARCH_POLICY_260815](./PLAN_SEARCH_POLICY_260815.md)가 대체한다
>
> `"찾아서 정리해보자"`에 검색이 붙지 않는 결함([DEV_260815](../logs/2026/08/DEV_260815.md))의 원인이
> 이 문서의 설계 자체에 있었다. 다음 절은 **더 이상 유효하지 않다**:
>
> | 절 | 내용 | 폐기 사유 |
> |---|---|---|
> | §6-2 | 게이트6(멀티턴 가드)이 게이트7(라우터 판정)보다 **위** | 가드가 사용자 명시 검색 요청을 이길 수 있다 → tier 100으로 강등 |
> | §4-2 | `explicit`을 `SEARCH_ON_PATTERNS`의 **한 태그**로 취급 | 명시 요청은 전용 계층(tier 300)이어야 한다. 이번 결함의 근본 |
> | §6-3 · §9-B | `prevSearched`를 직전 메시지 정규식으로 **근사**(A안) | 근사가 오발의 원인 → 실제 검색 여부 영속화로 대체 |
> | §7 | 비변경 경계에 **"클라이언트 코드"** 포함 | 위 영속화 때문에 플래그 1개 전달이 필요 |
>
> **여전히 유효한 것**: §1 핵심 통찰(검색 필요 여부는 13-intent와 직교), §5 라우터 LLM 합승(추가 홉 0),
> §3 intent별 제약표, §8 기대 효과. 재설계는 이 통찰들 **위에** 계층 구조를 얹는 것이지 뒤집는 게 아니다.
