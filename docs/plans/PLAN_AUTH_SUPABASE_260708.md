...
> 작성: 2026-07-08 · 갱신: 2026-07-09(코드·Supabase 보안 체크리스트 대조 재검토 반영) · 상태: **전체 설계(참조용)**
> 🚀 **실행은 [PLAN_AUTH_MVP_260709.md](PLAN_AUTH_MVP_260709.md)부터** — 익명로그인 + Google + RLS로 범위를 좁힌 MVP. 세션 전달은 MVP에서 **Bearer 토큰으로 확정**(§3의 A/B 결정 포인트 종결). 이 문서는 Kakao·이메일·Storage prefix 등 후속 범위의 참조 설계로 남는다.
