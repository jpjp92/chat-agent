...
| P1 | **인증 dev 이관 런북** | [PLAN_AUTH_DEV_ROLLOUT_260716.md](PLAN_AUTH_DEV_ROLLOUT_260716.md) | **로컬 선검증 완료 (2026-07-19) — dev 머지만 남음** — 스테이징을 dev 상시 DB 로 승격(main DB 미변경). env 통일·버킷 3개 생성·dev 도메인 OAuth·chat-agent-dev env 4개 stg 전부 완료. 로컬 3건(승계·빈게스트 로그인·업로드) 통과, 검증 중 버그 3개 수정([DEV_260719](../logs/2026/07/DEV_260719.md)) | **D. `feat/auth-mvp`→dev 머지**(커밋 후) → chat-agent-dev.vercel.app 스모크 |
