...
| P1 | **인증 dev 이관 런북** | [PLAN_AUTH_DEV_ROLLOUT_260716.md](PLAN_AUTH_DEV_ROLLOUT_260716.md) | **실행 대기 (2026-07-16)** — dev·main 이 프로덕션 DB 공유 → 스키마 변경은 DB 단위라 dev 머지가 곧 main 을 건드린다. 결정: **스테이징을 dev 상시 DB 로 승격**(main DB 미변경). env split-brain(서버는 아직 프로덕션) 통일 + 스테이징 버킷 생성 + dev 도메인 OAuth 등록이 선행 | 순서 B·C→A′→D. 🔴 스테이징 스토리지 버킷 존재 여부 먼저 확인 |
