...
| P3 | 날씨 전용 툴 (KMA+OWM) | [PLAN_WEATHER_TOOL_260706.md](PLAN_WEATHER_TOOL_260706.md) | **설계 기획 — 구현 대기**. 실험 검증 완료([DEV_260705](../logs/2026/07/DEV_260705.md)): KMA키=API Hub authKey·`dfsXyConv` 공식 격자 일치·레이턴시 1s. 카드 프리뷰([weather-card-sample.html](../../preview/weather-card-sample.html)) 라이트/다크+4개언어. grounding 15s+ → 결정론 카드 | `weather-tool.ts` 배선(dfsXyConv·육상생략·강수파싱) → router intent 분리 → WeatherRenderer. [TODO §P3 ⓪](../TODO.md) |
