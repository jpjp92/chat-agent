...
| 📄 Paper-Viz         | `paper_search` / `arxiv_search` | PubMed E-utilities + CrossRef (biomedical) · arXiv API (formal/physical/computational). The router's `paper_source` picks one, or answers without a card when neither database covers the field. arXiv terms are ANDed, so the tool schema forbids padding the query with generic words the user never said; a query carrying stopwords is tried as a **quoted phrase first** (a paper title is not a keyword bag) and falls back to the AND assembly |
